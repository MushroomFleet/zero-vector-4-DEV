...
class ZV4ApiClient {
  ...
  async makeRequest(method: string, endpoint: string, data?: any, params?: any) {
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new McpError(
          ...
          `ZV4 API Error: ${error.response?.data?.detail || error.message}`
        );
      }
      throw error;
    }
...
class ZeroVector4Server {
  ...
  private apiClient: ZV4ApiClient;
